from trading_gym.broker import BrokerEnv
...
while True:  # Run until solved
    ...
    for timestep in range(1, max_steps_per_episode):
        ...
        if frame_count < epsilon_random_frames or epsilon > np.random.rand(1)[0]:
            # Take random action
            selection = np.random.choice(num_actions)
        else:
            ...
            # Take best action
            selection = tf.argmax(action_probs[0]).numpy()
        ...
        state_next, reward, done, _ = env.step([("MSFT", selection)])
        state_next = feature_gen.generate(state_next)

        episode_reward += reward

        # Save actions and states in replay buffer
        action_history.append(selection)
